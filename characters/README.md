...
`npm run character:production:verify` is fail-closed. A reviewed candidate is never rewritten to make its evidence fit a later commit. The manual production workflow accepts the exact `candidate_sha` plus completed Illustrator and human brand-review evidence as external evidence, checks out that immutable candidate, validates both evidence documents against it, and only then runs `npm run character:release:promote` to derive release metadata.

Promotion changes `characters/release.json` to `status: "release"`, marks the two external gates verified, and writes `characters/production-attestation.json`. The attestation cryptographically binds the reviewed candidate commit, pre/post-promotion release manifests, both evidence files, the truth boundary, and the verified Rive runtime. `npm run character:production:verify` rechecks those bindings and rejects missing, stale, tampered, or candidate-mismatched evidence.

The manual GitHub workflow `.github/workflows/character-release.yml` keeps core QA on the GA Ubuntu runner, runs the pinned Rive runtime QA separately on Ubuntu 26.04, promotes only after both jobs succeed, exports the 756 governed delivery assets, packs the npm candidate, emits SHA-256 evidence manifests, and uploads a production-candidate artifact. It does not publish or merge automatically.
